...
public class Wallet extends Interface implements MouseWheelListener {
    ...
    public void render(Graphics2D g) {
        ...
        if (Constants.accounts.size() > 0 && Constants.accounts.get(selectedId).transactions.size() > 0) {
            for (int i = 0; i < Constants.accounts.get(selectedId).transactions.size(); i++) {
                if (180 + i * 70 - scrollOffset < Engine.getHeight() && 180 + i * 70 - scrollOffset > 80) {
                    ...
                    //Category
                    switch (Constants.accounts.get(selectedId).transactions.get(i).get("category").toString()) {
                        case "send":
                    ...
                    if (Constants.accounts.get(selectedId).transactions.get(i).get("address") != null) {
                        g.drawString(
        ...
        if (Constants.accounts.size() > 0) {
            ...
            g.drawString(Constants.getLangValue("Locked-Label"), Engine.getWidth() - (headerThird), 125);
            ...
            g.drawString(Constants.accounts.get(selectedId).lockedBalance, Engine.getWidth() - (headerThird) + 30 - (g.getFontMetrics().stringWidth(Constants.accounts.get(selectedId).lockedBalance) / 2), 145);
        }
